...
def jelszo_szam(db):
	...
	tomb="123456789"
	for x in range (0,db):
		szoveg+=tomb[random.randint(0,len(tomb)-1)]
	return szoveg
...
def jelszo_spec(db):
	...
	tomb2="*>;<@&#~"
	for x in range (0,db):
			szoveg2+=tomb2[random.randint(0,len(tomb2)-1)]
	return szoveg2
...
def jelszo_nagybetu(db):
	...
	for x in range (0,db):
		szoveg3+=tomb3[random.randint(0,len(tomb3)-1)]
	return szoveg3
...
def jelszo_kisbetu(db):
	...
	for x in range (0,db):
		szoveg4+=tomb4[random.randint(0,len(tomb4)-1)]
	return szoveg4
